# /----------------------------------------------------------------------------------------------\
#     This code is a Image Resizer code created in Python language - version 3.12 or higher
...
def resize_multiple_images(folder_path: str, new_dimensions: tuple[int, int]) -> None:
    ...
    for file_name in os.listdir(folder_path):
        if os.path.splitext(file_name)[-1] in supported_extensions:
            resize_image(os.path.join(folder_path, file_name), new_dimensions)
    ...
    for file_name in os.listdir(HERE_PATH):
        base_name: str = os.path.basename(file_name)

        try:
            if "copy-" in base_name[:base_name.rindex('.')]:
...
def _main(args: Any = None) -> None:
    ...
    print("\n• Done! Press any key to close...", end='')
